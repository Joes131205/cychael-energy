...
const DashboardPage = () => {
  ...
  return (
    <ScrollView className="flex-1" style={{backgroundColor: colors.background}}>
      <LinearGradient
        colors={isDarkMode ? ["#1A2E2A", "#121C1A"] : ["#283F3B", "#99DDC8"]}
        start={{x: 0, y: 0}}
        end={{x: 1, y: 1}}
        ...
        <View className="bg-white/10 rounded-[15px] p-5 mb-3">
          <Text className="text-white text-base font-bold mb-4">
            ...
            <Text className="text-white/70 text-xs font-normal">
              {"\n"}Last updated:
              {new Date(userData?.deviceList?.updatedAt).toLocaleDateString(
                "en-US",
                {
                  weekday: "short",
                  month: "short",
              ...
              at
              {new Date(userData?.deviceList?.updatedAt).toLocaleTimeString(
                "en-US",
                {
                  hour: "2-digit",
                  minute: "2-digit",
        ...
        <View
          ...
          >
            {deviceCategories.length === 0 ? (
              ...
              deviceCategories.slice(0, 10).map((device, index) => (
                <View
                  key={index}
          ...
          {deviceCategories.length > 3 && (
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate("EnergyAnalysisResultPage")}>
              <Text
